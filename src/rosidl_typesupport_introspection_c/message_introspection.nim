...
import
  rosidlRuntimeC/messageInitialization, rosidlRuntimeC/messageTypeSupportStruct,
  rosidlRuntimeC/visibilityControl, rosidlRuntimeC/visibilityControl,
  rosidlRuntimeC/messageTypeSupportStruct,
  rosidlRuntimeC/messageTypeSupportStruct, ./visibility_control,
  ./visibility_control
...
type
  ...
  RosidlTypesupportIntrospectionCMessageMember* {.
    ...
    arraySize* {.importc: "array_size_".}: csize_t ##  If is_array_ is true, this contains the number of members in the array.
    ...
    getFunction* {.importc: "get_function".}: proc (a1: pointer; index: csize_t): pointer ##
                              ...
                              ##  array indicated by index.
    fetchFunction* {.importc: "fetch_function".}: proc (a1: pointer; index: csize_t;
        a3: pointer) ##  Pointer to a function that fetches (i.e. copies) an item from
    ...
    assignFunction* {.importc: "assign_function".}: proc (a1: pointer; index: csize_t;
        a3: pointer) ##  Pointer to a function that assigns (i.e. copies) a value to an
                   ...
                   ##
                   ##  Available for array and sequence members.
    resizeFunction* {.importc: "resize_function".}: proc (a1: pointer; size: csize_t): bool ##
  ...
  RosidlTypesupportIntrospectionCMessageMembers* {.
    ...
    memberCount* {.importc: "member_count_".}: uint32 ##  The number of fields in the interface
    ...
    initFunction* {.importc: "init_function".}: proc (a1: pointer;
        a2: RosidlRuntimeCMessageInitialization) ##  The function used to initialise the interface's in-memory representation
